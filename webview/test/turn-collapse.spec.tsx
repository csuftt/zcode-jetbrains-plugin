/**
 * 完成轮折叠回归测试（MessageBubble AssistantBubble，2026-08-29）
 *
 * 行为（对齐官方客户端）：
 *   - 非流式的完整轮默认折叠：只渲染最终结论（最后一个 text part），执行过程不进 DOM
 *   - 点击底部「⏱ 已工作 X」按钮展开/收起过程
...
describe('完成轮折叠', () => {
  ...
  it('完成轮默认折叠：只渲染结论，工具卡不进 DOM，耗时变为按钮', () => {
    ...
    expect(btn.textContent).toContain('已工作')
    // 10 分 33 秒（completed - created）
