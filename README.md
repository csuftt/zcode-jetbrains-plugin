...
**对话** — 流式输出（思考过程 / 正文 / 工具调用实时渲染）、Markdown / Mermaid / 代码高亮、思考耗时统计、消息排队（生成中回车自动排队，排队卡片可立即发送 / 删除 / 引导插队——不打断生成，AI 在回合中途收到指令调整方向，被引导消息带 ⚡ 徽标）、最后一轮用户消息可编辑重发（修改后自动回退该轮重新生成）、会话内定时发送（预设时间 + 提示词到点自动执行，支持指定模型 / 新会话执行，任务列表跨会话管理）、Ctrl+F 会话内搜索（大小写 / 整词 / 正则）、消息锚点导航（用户消息圆点定位 + hover 预览）

**多任务** — 多标签页并行会话（每标签独立上下文互不串扰）、重启 IDE 自动恢复、会话列表 / 重命名（支持 AI 根据最近对话重新起标题）/ 搜索 / 批量多选删除、不常用会话归档收起（支持自动归档）、任意历史回复一键分叉新会话（任务执行中也可）

**过程可视** — 任务清单（TodoWrite）实时进度、子代理（Agent）面板与执行过程 / 最终报告弹窗、文件改动统计（点击在编辑器打开、行内 diff 前后对比）、AskUserQuestion 交互弹窗（底部停靠可最小化、默认一直等待回答、回合结束可回看当时的问答）、计划模式（ExitPlanMode）审批弹窗

**目标模式** — `/goal` 设定长目标后自动多轮推进：每轮工作结束服务端独立校验一次（时间线分隔卡显示通过与否与下一步行动），未通过自动续跑直至目标达成；右上角目标卡实时显示轮次 / 用时 / 校验中状态，支持暂停 / 恢复 / 换目标 / 二次确认清除，目标与轮次进度跨重启恢复

**内嵌浏览器** — Header 一键在聊天区右侧展开浏览器分栏：多 tab（全局共享、跨会话沿用）、后退 / 前进 / 刷新 / 地址栏 / 自由尺寸（DevTools 设备工具栏形态的虚拟屏）/ DevTools / 外部打开，浏览器控制台（CDP）支持清理站点数据与概览查看；插件作为宿主实现 browser-use 反向协议，AI 无需任何配置即可驱动这方浏览器导航、截图、执行 JS、跑 playwright 定位器与 CUA 鼠标键盘操作

**运行时控制** — 模型下拉切换（内置渠道跟随 ZCode 客户端当前生效渠道，下拉内可手动刷新）、权限模式（build / edit / plan / yolo）与思考级别（随模型动态）调整，待命态（未建会话）可预选、建会话即生效；上下文容量圆环（含用量构成与缓存命中）、5 小时 / 每周额度查询

**设置中心** — 七页签：基础（主题 / 字体 / 语言 / 自定义配色 + 环境路径）、模型（内置渠道以 ZCode 客户端配置为准、只展示当前生效渠道并标注命中方式（客户端选中 / 兜底生效），内置渠道支持自定义 API Key——团队套餐从 bigmodel 开放平台拿项目 key 填入即走团队额度，卡片显示当前实际计费的 key（默认脱敏、可看全部）与来源，未配置时给出计费提醒；第三方供应商可启停；路径跟随数据目录迁移，新增 / 删除引导前往 Zcode 配置并可一键打开配置文件）、用量（应用用量：本地会话统计，覆盖第三方模型，7 天 / 30 天 / 全部区间；GLM 套餐用量：额度卡片 + 模型 / 工具用量曲线与明细表，标注查询凭证来源渠道与所用 key（脱敏））、记忆（AGENTS.md 指令记忆 + 自动记忆，缺失可创建）、技能（全局 / 项目 / 插件三来源扫描，行内启用禁用）、MCP（服务器清单 / 工具列表 / 连接日志）、其他（输入历史补全开关与历史记录管理）

**环境检测** — 启动自检 Node.js（≥18）/ ZCode CLI / 登录凭证三件套，异常时顶栏提醒条逐项给出修复入口与重新检测（凭证缺失不阻断启动，仅提示补配）；路径可手动配置，留空自动探测

**IDE 集成** — 项目视图 / 编辑器标签右键发送文件、编辑器右键发送选中代码到输入框（Ctrl+Alt+K）、复制选区引用（路径 + 行号）；文件、记忆、技能、MCP 配置均可一键在编辑器打开

**输入增强** — `@` 引用文件（chip + 补全，粘贴绝对路径或从系统拖入文件自动转 chip）、`/` 调用技能、长文本粘贴折叠、输入历史回溯与前缀幽灵补全（Tab 采纳）
...
| 多轮推进中：每轮校验分隔卡（未通过 → 下一步行动）+ 目标卡轮次 / 用时 / 校验中 | 全部轮次完成：最终校验通过，目标卡转完成态 |
| :---: | :---: |
| ![目标模式运行中](docs/screenshots/goal-processing.png) | ![目标模式完成](docs/screenshots/goal-done.png) |
...
| 基础设置（主题 / 字体 / 语言 / 自定义配色 + 环境路径） | 模型管理（内置渠道只读 + 命中方式徽章 / 第三方启停） |
| :---: | :---: |
| ![基础设置](docs/screenshots/settings-basic.png) | ![模型管理](docs/screenshots/settings-models.png) |
| **用量查询（应用用量：本地统计 + 第三方模型明细）** | **记忆（指令 / 自动记忆管理）** |
